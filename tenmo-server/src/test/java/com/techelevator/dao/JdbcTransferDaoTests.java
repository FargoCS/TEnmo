...
public class JdbcTransferDaoTests extends BaseDaoTests {
    ...
    public void decline_updates_transfer_status() {
        Transfer createdRequest = sut.createRequest(TRANSFER_3);
        Transfer actual = sut.decline(createdRequest);
        Assert.assertNotEquals(0,actual.getTransferId());
        Assert.assertEquals(TRANSFER_3.getTransferTypeId(), actual.getTransferTypeId());
        Assert.assertEquals(3, actual.getTransferStatusId());
        Assert.assertEquals(TRANSFER_3.getAccountFrom(), actual.getAccountFrom());
        Assert.assertEquals(TRANSFER_3.getAccountTo(), actual.getAccountTo());
        Assert.assertEquals(TRANSFER_3.getAmount(), actual.getAmount());
    }
}